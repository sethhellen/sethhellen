# Hellen Seth

**Senior Graphic Designer · Digital Strategist · Scrum Master**  
Content & Marketing · Nairobi, Kenya 🇰🇪

---

I design brands, build digital strategies, and lead creative teams  with a growing obsession for AI-integrated workflows that make both better.

My work lives at the intersection of **visual identity**, **content strategy**, and **agile execution**. I bring Scrum discipline to creative processes that usually resist it, and I bring a designer's eye to strategy decks that usually don't have one.

---

## What I Do

| Area | What it looks like |
|---|---|
| 🎨 Brand Identity | Logo systems, visual language, brand guidelines |
| 📣 Campaign Design | End-to-end creative for digital & print campaigns |
| 🤖 AI-Integrated Workflows | Prompt-driven content pipelines, AI-assisted design production |
| 📱 Content Strategy | Positioning, content calendars, social & editorial strategy |
| 🎬 Film & Animation | Diploma in Film Production & Animation — Multimedia University |
| ⚙️ Scrum Mastery | Certified Scrum Master — agile delivery for creative teams |

---

## Tools & Stack

**Design**
`Adobe Creative Suite` · `Figma` · `Canva Pro`

**Strategy & Content**
`Notion` · `Trello` · `Microsoft Teams` · `HubSpot`

**AI Workflow**
`Claude` · `ChatGPT` · `Midjourney` · `ElevenLabs`

**Dev-Adjacent**
`HTML/CSS` · `GitHub` · `Webflow`

---

## Currently

- 🏗️ Leading content & marketing at delivering transformation strategy for enterprise clients across the GCC
- 🌍 Exploring remote-first opportunities with global product teams
- 🚗 Building side projects at the intersection of design and technology

---

## Featured Work

> *Portfolio available on request reach out below.*

---

## Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Hellen_Mweu-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/hellenmweu)
[![Email](https://img.shields.io/badge/Email-hellenmweu%40gmail.com-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:hellenmweu@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-View_Work-111111?style=flat&logo=data:image/svg+xml;base64,&logoColor=white)](https://hellenmweu.com)

---

<sub>Designer by craft · Strategist by instinct · Always building something.</sub>
